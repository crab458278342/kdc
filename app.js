function setLogoBut(a) {
    logoData.image && (logo && document.body.removeChild(logo), logo = document.createElement("img"), logo.src = logoData.image, logo.addEventListener ? (logo.addEventListener("click", logoData.action), logo.addEventListener("touchend", logoData.action)) : logo.attachEvent && (logo.attachEvent("click", logoData.action), logo.attachEvent("touchend", logoData.action)), logo.className += " spilgames-branding-image-" + a, document.body.appendChild(logo))
}
function initSplash() {
    gameState = "splash",
    resizeCanvas(),
    1 != audioType || muted || music.play(),
    spilApiInstance.Branding.displaySplashScreen(initStartScreen)
}
...
function butEventHandler(a, b) {
    ...
    case "startGame":
        ...
        initGame(),
        levelNum > 0 && GameAPI.GameBreak.request(adPauseOn, adPauseOff);
        break;
    case "backFromMap":
        playSound("click"),
        userInput.removeHitArea("startGame"),
...
spilApiInstance,
SpilData = {
    id: "576742227280292741"
};
GameAPI.loadAPI(function(a) {
    spilApiInstance = a,
    window.console && window.console.log && console.log("GameAPI version " + a.version + " loaded!"),
    logoData = a.Branding.getLogo(),
    buttonProperties = a.Branding.getLink("more_games"),
    loadPreAssets()
},
SpilData);